...
const languages = readdirSync(join(dir, 'content')).filter(name => !name.includes('.'))

languages.forEach(language => {
  const pages = globSync(join(dir, 'content', language, '**', '*.md')).map(filePath => {
    const pageData = getMarkdown(filePath)
    if (language !== 'default') pageData.lang = language
    pageData.permalink = relative(join(dir, 'content'), filePath).replace('.md', '').replace(language + '/', '')
    pageData.id = pageData.permalink
    return pageData
  })
  
  const posts = globSync(join(dir, 'content', language, 'blog', '*.md')).map(filePath => {
